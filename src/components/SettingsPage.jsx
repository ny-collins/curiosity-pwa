...
import { X, ArrowLeft, AlertTriangle, Upload, Download, CheckCircle, BellRing, LogIn, User, FileOutput } from 'lucide-react';
...
const fontOptions = [
    ...
    { name: 'Slab Serif', value: "var(--font-slab)" },
    { name: 'Cursive', value: "var(--font-cursive)" },
];
...
function SettingsPage({
    ...
    const [activeFont, setActiveFont] = useState(initialSettings.fontFamily || "var(--font-sans)");
    ...
    useEffect(() => {
        if(initialSettings) {
            ...
            setActiveFont(initialSettings.fontFamily || "var(--font-sans)");
        }
        ...
        <>
            <div className="flex flex-col h-full bg-slate-800 overflow-hidden">
                <div className="p-4 border-b border-slate-700 flex justify-between items-center space-x-2 flex-shrink-0">
                    <button onClick={onBack} className="p-2 -ml-2 rounded-full text-gray-400 hover:text-white hover:bg-slate-700 md:hidden focus:outline-none focus:ring-2" style={{'--tw-ring-color': 'var(--color-primary-hex)'}} aria-label="Back to list" title="Back to list">
                        <ArrowLeft size={22} />
                    </button>
                    ...
                    <div className="max-w-xl mx-auto space-y-8"> 
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {themeColors.map(color => (
                                        <button
                                            ...
                                            className={`w-8 h-8 rounded-full cursor-pointer focus:outline-none transition-transform duration-100 ${activeThemeColor === color.hex ? 'ring-2 ring-white ring-offset-2 ring-offset-slate-800 scale-110' : 'hover:scale-110'}`}
                                            style={{ backgroundColor: color.hex, '--tw-ring-color': 'var(--color-primary-hex)' }}
                                        />
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {fontOptions.map(font => (
                                        <button
                                            ...
                                            className={`py-1 px-3 rounded-md text-sm transition-colors ${activeFont === font.value ? 'text-white font-semibold' : 'bg-slate-700 text-gray-300 hover:bg-slate-600'}`}
                                            style={{ 
                        ...
                        <div className="border-t border-slate-700 pt-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="username" value={username} onChange={(e) => setUsername(e.target.value)} className="form-input w-full bg-slate-700 text-white rounded-md border-slate-600" placeholder="Your Name" />
                            </div>
                            <div>
                                <label htmlFor="profilePicUrl" className="block text-sm font-medium text-gray-300 mb-1">Profile Picture URL</label>
                                <input type="text" id="profilePicUrl" value={profilePicUrl} onChange={(e) => setProfilePicUrl(e.target.value)} className="form-input w-full bg-slate-700 text-white rounded-md border-slate-600" placeholder="https://your-image-url.com/pic.png" />
                            </div>
                             <div>
                                  ...
                                  <button
                                    ...
                                    className="w-full flex items-center justify-center space-x-2 bg-slate-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-slate-600 transition-colors duration-200 focus:outline-none focus:ring-2 disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            {enableLock && (
                                <div>
                                    ...
                                    <input type="password" id="pin" value={pin} onChange={(e) => { const val = e.target.value.replace(/\D/g, ''); if (val.length <= 4) setPin(val); }} maxLength={4}
                                        className="form-input w-full bg-slate-700 text-white rounded-md border-slate-600 tracking-widest"
                                        placeholder="••••" />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="border-t border-red-500/30 pt-6 space-y-4">
                            <h3 className="text-lg font-medium text-red-500">Danger Zone</h3>
                             <div className="flex items-center justify-between">
                                <div>
                                    <h4 className="font-semibold text-gray-200">Export All Data</h4>
                                    <p className="text-xs text-gray-400">Download a JSON file of all your entries and reminders.</p>
                                </div>
                                <button
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400">Permanently delete all your data from the cloud.</p>
                                </div>
                                <button
